    ...
    public class Analyzer
    {
        ...
        */
        public bool Start(String[] args)
        {
            ...
            TypeRepository.Instance.CompleteUrlForEachScope(TypeRepository.Instance.RootScope);
            TypeRepository.Instance.CompleteStructInfo(TypeRepository.Instance.RootScope);
            ...
            if (cmdp.R)
            {
                ...
                OutputManager.DisplayRelationshipForClass(TypeRepository.Instance.RootScope);
            }
            /*
             * Write XML
             *  
            ...
            if (cmdp.X)
            {
                if(cmdp.XMLName.Length==0)
                    OutputManager.WriteTypes2XML(TypeRepository.Instance.RootScope, "a.xml",cmdp.R);
                else
                    OutputManager.WriteTypes2XML(TypeRepository.Instance.RootScope, cmdp.XMLName,cmdp.R);
            }
            return true;
        }
    }
